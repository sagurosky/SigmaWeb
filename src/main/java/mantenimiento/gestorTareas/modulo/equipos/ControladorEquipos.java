...
public class ControladorEquipos {
    ...
    public String ponerADisponibilidad(Model model, Activo activoRequest) {
        ...
        if (activoSeleccionado != null) {
            ...
            // Si la fecha de inicio es igual o anterior al momento actual, se activa
            // inmediatamente
            if (!desde.isAfter(ahora)) {
                ...
                // Si es futura, permanece en estado "operativa" hasta que el
                // DisponibilidadScheduler la active
    ...
    public String cancelarDisponibilidad(Model model, Activo activoRequest) {
        Activo activoSeleccionado = activo.findById(activoRequest.getId()).orElse(null);
        if (activoSeleccionado != null) {
            List<Tarea> tareasDisponibles = tareaService.traerDisponiblePorActivo(activoSeleccionado,
            ...
            // Reset only the start of availability; keep end date for future scheduling
            activoSeleccionado.setEstado("operativa");

            activoSeleccionado.setDisponibilidadDesde(null);
            // Also clear the end date to fully cancel the availability window
    ...
    public String notificarDetencionCondicionada(Model model, Activo activoRequest) {
        Activo activoSeleccionado = activo.findById(activoRequest.getId()).orElse(null);
        if (activoSeleccionado != null) {
            List<Tarea> tareasNoCerradas = tareaService.traerNoCerradaPorActivo(activoSeleccionado,
                    TenantContext.getTenantId());
    ...
    public String cancelarDetencion(Model model, @PathVariable Long id) {
        ...
        if (activoSeleccionado != null) {
            ...
            boolean esAdmin = (aut != null)
                    && aut.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN"));
            ...
            if (tareasNoCerradas != null && !tareasNoCerradas.isEmpty()) {
                for (Tarea tarea : tareasNoCerradas) {
                    if (esAdmin
                            || (tarea.getSolicita() != null && tarea.getSolicita().equalsIgnoreCase(usuarioActual))) {
                        puedeCancelar = true;
            ...
            if (puedeCancelar) {
                ...
                // Si la ventana de disponibilidad programada ya comenzó, activarla inmediatamente
                LocalDateTime ahora = TiempoUtils.ahora();
                LocalDateTime disponibilidadDesde = activoSeleccionado.getDisponibilidadDesde();
                LocalDateTime disponibilidadHasta = activoSeleccionado.getDisponibilidadHasta();
                if (disponibilidadDesde != null && !disponibilidadDesde.isAfter(ahora)
                        && (disponibilidadHasta == null || disponibilidadHasta.isAfter(ahora))) {
                    // No hay más tareas abiertas (acabamos de cancelar la única)
                    activoSeleccionado.setEstado("disponible");
    ...
    private void cargarModel(Model model, Activo activo) throws IOException {
        ...
        model.addAttribute("linkFoto", "/inicio/recursos/imagenes/" + activo.getNombreCamelCase().replace(" ", "")
                + "Tenant" + TenantContext.getTenantId() + ".jpg");
        ...
        // lógica para llevar los ultimos 12 meses anteriores al mes actual para
        // graficar los minutos de detencion de cada mes
        LocalDate fechaActual = LocalDate.now();

        // aca es donde tengo que determinar el tipo de calculo, si es promedio movil
        // por dias o detenciones, si es de acuerdo al ultimo periodo....
        // una vez definido traigo las tareas correspondientes, hay que programarlo en
        // los Dao.

        // promedioMovil mes, 3 meses, 6 meses, 1 año. ver como configurarlo, lo
        // hardcodeo a un año
        ...
        for (int i = 11; i >= 0; i--) {
            ...
            for (Tarea tarea : tareasUltimoAnio) {
                if (tarea.getMomentoDetencion() != null && tarea.getMomentoLiberacion() != null
                        && tarea.getMomentoDetencion().isAfter(mesAnterior)
                        && tarea.getMomentoDetencion().isBefore(finMesAnterior)) {
                    minutosDetencion += Duration.between(tarea.getMomentoDetencion(), tarea.getMomentoLiberacion())
                            .toMinutes();
                }
            }
        ...
        if (activo.getPromedioMovil() == null)
            activo.setPromedioMovil("1 anio");
        switch (activo.getPromedioMovil()) {
            case "1 anio":
                desde = TiempoUtils.haceAnios(1);
                break;
            case "6 meses":
                desde = TiempoUtils.ahora().minusMonths(6);
                break;
            case "3 meses":
                desde = TiempoUtils.ahora().minusMonths(3);
                break;
            case "1 mes":
                desde = TiempoUtils.ahora().minusMonths(1);
                break;
        }

        // para que calcule bien los indicadores al principio le pongo la fecha de
        ...
        if (inicioDeActividades != null) {
            if (desde.isBefore(inicioDeActividades))
                desde = inicioDeActividades;
        }
        ...
        // confiabilidad: e elevado a (-t/mtbf) resultado multiplicado por 100 para
        // tenerlo en porcentaje
        ...
        boolean esAdmin = (aut != null)
                && aut.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN"));
        boolean puedeCancelarDetencion = esAdmin || (tareaActivo != null && tareaActivo.getSolicita() != null
                && tareaActivo.getSolicita().equalsIgnoreCase(usuarioActual));
