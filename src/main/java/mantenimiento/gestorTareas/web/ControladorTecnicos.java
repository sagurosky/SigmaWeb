...
//        model.addAttribute("tareas", tareas);
    ...
    public String verPerfilTecnico(@RequestParam("id") Long id, @RequestParam("url") String url, Model model) {
        ...
        Integer preventivosMes=preventivoService.traerPorTecnicoEnRangoDeFecha(tecnico, LocalDateTime.now().minusMonths(1).toString(), LocalDateTime.now().toString()).size();
        Integer preventivosAnio=preventivoService.traerPorTecnicoEnRangoDeFecha(tecnico, LocalDateTime.now().minusYears(1).toString(), LocalDateTime.now().toString()).size();
        ...
        Integer informesMes=informeService.traerPorTecnicoEnRangoDeFecha(tecnico, LocalDateTime.now().minusMonths(1).toString(), LocalDateTime.now().toString()).size();
        Integer informesAnio=informeService.traerPorTecnicoEnRangoDeFecha(tecnico, LocalDateTime.now().minusYears(1).toString(), LocalDateTime.now().toString()).size();
        
        Integer informesPendientes=tareaService.traerPorTecnicoYEstadoInforme(tecnico,"pendiente").size()+tareaService.traerPorTecnicoYEstadoInforme(tecnico,"EnRevision").size();
